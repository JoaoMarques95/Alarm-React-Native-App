function diff_minutes(Last_Click, Actual_Click) {
        ...
        const Actual_Click_Date = new Date(Actual_Click); // make the adjustments to make the string
        const Last_Click_Date = new Date(Last_Click); // make the adjustments to make the string
        const diff = Math.abs(Actual_Click_Date - Last_Click_Date); // milliseconds between actual click & Last one!
                                        ...
                                        if (Data.State === 'Pressed') {
                                                User.find({ Topic: topic }) // filter the topic
                                                        ...
                                                        .exec(function(err, doc) {
                                                                ...
                                                                const Last_click = doc[0].Clicks[0];
                                                                const Actual_click = Data.Time; // aranjar maneira de as strings ficarem no formato indicado!
